...
class Uniform15KPC(Dataset):
    def __init__(self, root_dir, subdirs, tr_sample_size=10000,
        ...
        print(self.all_points_std.shape)
...
class ShapeNet15kPointCloudsW(ShapeNet15kPointClouds):
    '''Wrapper for ShapeNet15kPointClouds that returns a dummy label'''
    def __getitem__(self, idx):
        res = super().__getitem__(idx)
        return res['train_points'], [1] # dummy label
    

if __name__ == "__main__":
    path = "/home/ioannis/Desktop/programming/data/ShapeNetCore.v2.PC15k"
    from torch.utils.data import DataLoader
    #shape_ds = ShapeNet15kPointClouds(root_dir=path, categories=['airplane'], split='train')
    ...
    train_dl = DataLoader(tr_dataset, batch_size=32, shuffle=True, num_workers=4)
    from utils.visualization import quick_vis_batch
    from functools import partial
    vis_batch = partial(quick_vis_batch, x_offset = 8, y_offset=8)
    vis_batch(next(iter(train_dl))['train_points'])
